...
export async function generateMetadata() {
  return getPageMetadata({
    title: "My Bookmarks",
    description: "The home to all your preferences and settings on Curious Programmer.",
    slug: "/my/bookmarks",
...
export default async function MyBookmarksPage() {
  ...
  const content = bookmarks.map((bookmark) => {
    return allContent.find((content) => bookmark.slug === content.slug);
        ...
        <div className="my-4 grid grid-cols-1 gap-4 lg:grid-cols-3">
          {sections.map((section) => (
            <Link
              key={section.href}
              href={section.href}
              className="flex gap-4 rounded-lg border border-gray-200 p-6 hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-800"
            >
